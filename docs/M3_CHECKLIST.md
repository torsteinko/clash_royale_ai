...
- [x] M3.5 Tikkende soner: poison/earthquake — implementert 1:1 (per-tick-skade via AreaEffectFactory-derivasjonen round(buff.damagePerSecond × hitSpeed), TickingHandler-akkumulator + levetidsrekkefølge, EnemyDamageApplicator: bygg-bonus ×4.5, crown-%, gulv 1, bakke/luft-filter). Enhetstester i test_m3 (poison 32×5 tårn / 23 per tick; quake 30×20 / 7). Java-verifisert via 2 nye M4-scenarier (poison_zone/earthquake_zone, byte-sammenligning): skade-ticks og verdier identiske; første avvik = buff-slow (DIVERGENCES #23, ikke modellert — bevisst utenfor M3.5)
- [x] M3.6 Log: spellAsDeploy-kjeden 1:1 (deploy-prosjektil i kastpunktet + rullende piercing sub-prosjektil med minDistance-gate, hit-once per enhet, direksjons-pushback 700 i 0,5 s, 15 % crown-skade). Nytt scenario `log_roll` er byte-identisk mot java_patched: knight-treff tick 115 (= gate-ticken; uten gate ville det truffet tick 113), tårn 15 % (40) tick 147, flygende MegaMinion uskadd, rullen deaktiverer etter 61 bevegelses-ticks. Fikset på veien: formasjons-offsets (rød speiler (-dx,-dy) + frame-flip i `deploy()`; funnet via røde minions), trace-unit-navn (unit-navn + java-normalisering, ikke kort-navn). DIVERGENCES #4 → DONE; #3 presisert (Java skiller luft-luft-klynger per tick — fler-enhets-deploys holdes derfor utenfor byte-settet til kollisjoner portes). Tester: 3 nye i test_m3 + pin i test_m4
...
(kjør alle: `for t in test_smoke test_combat test_projectiles test_pathing test_m3 test_vecenv test_m4 test_sb3; do python3 -m gpusim.tests.$t || break; done` — 55 tester)
