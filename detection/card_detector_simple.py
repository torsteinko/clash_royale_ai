...
class CardDetectorSimple:
    ...
    def _detect_card_at_position(
        ...
        """Detect a card at a specific position using color matching only"""
        ...
        # Find best matching template using COLOR only
        best_name, best_score = self._find_best_match(card_artwork)

        return {"name": best_name, "score": best_score, "method": "color"}

    def _find_best_match(self, card_artwork: np.ndarray) -> Tuple[str, float]:
        """Find the best matching template using color matching"""
    ...
    def _apply_persistence_logic(self, slot_idx: int, result: dict) -> dict:
        ...
        if locked_card != "unknown" and not self._is_waiting_for_card(locked_card):
            ...
            if detected_name == locked_card:
                ...
                # Different card detected without waiting_for_card
                # Check if it's actually waiting_for_card (we might have the best match wrong)
                # If the new detection is NOT waiting_for_card, assume we missed it
                if detection_info["card"] == detected_name:
                    detection_info["count"] += 1
                else:
                    detection_info["card"] = detected_name
                ...
                if detection_info["count"] >= self.CONFIDENCE_THRESHOLD:
                    # Force enter waiting state first, then transition
                    waiting_state["active"] = True
                    waiting_state["frames"] = 0
                    self._locked_cards[slot_idx] = "unknown"
                    return {
...
# Backwards-compatible alias for modules expecting `CardDetector`
