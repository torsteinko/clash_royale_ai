...
class TowerDetector:
    ...
    def __init__(self):
        ...
        for tower_name in self.TOWER_POSITIONS.keys():
            self.tower_states[tower_name] = {
                'alive': True,
                'type': None,           # Unknown until identified
                'hp': None,
                'max_hp': None,
                'level': None,
                'identified': False     # Becomes True after first identification
            }
            ...
            if hp is not None:
                ...
                if not state['identified'] and level is not None:
                    ...
                    if tower_type != 'unknown':
                        ...
                        print(f"✅ Identified {tower_name}: {display_name} (Lvl {level}, {hp} HP)")
    ...
    def _read_hp(self, frame: np.ndarray, region: Tuple[int, int, int, int]) -> Optional[int]:
    ...
    def _read_level(self, frame: np.ndarray, region: Tuple[int, int, int, int]) -> Optional[int]:
        """Read level value from region"""
        ...
        try:
            ...
            # Parse
            level = int(''.join(filter(str.isdigit, text)))
    ...
    def get_tower_summary(self) -> Dict:
        ...
        ally_alive = sum(1 for name, state in self.tower_states.items() 
                        if 'ally' in name and state['alive'])
        enemy_alive = sum(1 for name, state in self.tower_states.items() 
